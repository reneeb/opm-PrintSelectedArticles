...
# Changes Copyright (C) 2014 - 2022 Perl-Services.de, https://perl-services.de
# --
...
# PS
# ---
    my @ArticlesToPrint = @{ $Param{Articles} || [] };
    if ( @ArticlesToPrint ) {
        @MetaArticles  = ();
        my %ArticleMap = map{ $AllArticles[$_]->{ArticleID} => $AllArticles[$_] } 0 .. $#AllArticles;

        for my $ArticleInfo ( @ArticlesToPrint ) {
            my ($ArticleID, $ArticleNumber) = split /__/, $ArticleInfo;

            my $Article = $ArticleMap{$ArticleID};
            $Article->{__PS_ArticleNumber} = $ArticleNumber;
$Kernel::OM->Get('Kernel::System::Log')->Log( Priority => error => Message => $ArticleNumber );

            push @MetaArticles, $Article;
        }
    }
# ---
...
# PS
# ---
        $Article{__PS_ArticleNumber} = $MetaArticle->{__PS_ArticleNumber};
# ---
...
#        if ( $Param{ArticleNumber} ) {
        if ( $Article{__PS_ArticleNumber} ) {
            $ArticleNumber = $Article{__PS_ArticleNumber};
        }
        elsif ( $Param{ArticleNumber} ) {
# ---
            $ArticleNumber = $Param{ArticleNumber};
        }
        else {
